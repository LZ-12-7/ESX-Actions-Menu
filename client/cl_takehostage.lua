...
function callTakeHostage()
	...
	if not canTakeHostage then 
		drawNativeNotification("Necesitas una pistola para apuntar a alguien en la cabeza!")
	end
	...
	if not takeHostage.InProgress and canTakeHostage then			
		...
		if closestPlayer then
			...
			if targetSrc ~= -1 then
				...
				drawNativeNotification("~r~No hay nadie cerca para hacerle th!")
			end
		else
			drawNativeNotification("~r~No hay nadie cerca para hacerle th!")
		end
...
Citizen.CreateThread(function()
	while true do 
		if takeHostage.type == "agressor" then
			...
			drawNativeText("Presione [G] para empujarlo, [H] para matarlo")
